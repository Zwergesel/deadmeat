  ...
  for(int r=0;r<roomNumber;r++)
  {
    ...
    if(roomType <= 80)
    {
      ...
      for(int i=0;i<numDoors;i++)
      {
        ...
        if(side == 0) {p.x = x + d; p.y = y - 1;}
        if(side == 1) {p.x = x - 1; p.y = y + d;}
        if(side == 2) {p.x = x + d; p.y = y + sizeY;}
        if(side == 3) {p.x = x + sizeX; p.y = y + d;}
        doors.push_back(p);
    ...
    else if(roomType <= 90)
    {
      ...
      for(int i=0;i<numDoors;i++)
      {
        Point p;
        int side = rng.getInt(0,3);
        if(side == 0) {p.x = x; p.y = y + size;}
        if(side == 1) {p.x = x + size; p.y = y;}
        if(side == 2) {p.x = x + 2*size; p.y = y + size;}
        if(side == 3) {p.x = x + size; p.y = y + 2*size;}
        doors.push_back(p);
        swap1[p.x + p.y * width] = 2;
      }
    }
    ...
    else
    {
      ...
      for(int i=0;i<numDoors;i++)
      {
        ...
        if(side == 0) {p.x = x; p.y = y + sizeY;}
        if(side == 1) {p.x = x + sizeY + d; p.y = y;}
        if(side == 2) {p.x = x + 2*sizeY + sizeX + 1; p.y = y + sizeY;}
        if(side == 3) {p.x = x + sizeY + d; p.y = y + 2*sizeY;}
        doors.push_back(p);
        swap1[p.x + p.y * width] = 2;
      }
    }
